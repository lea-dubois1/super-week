...
class AuthController
{
    ...
    public function register($email, $firstname, $lastname, $password): string
    {
        ...
        if($row < 1) {
            ...
            $model->insert(['email' => $email, 'first_name' => $firstname, 'last_name' => $lastname, 'password' => $hash]);
    ...
    public function login($email, $password)
    {
        ...
        if($row > 0) {
            ...
            $DBPass = $model->select(["password"], ['email' => $email])[0]['password'];

            if(password_verify($password, $DBPass) === true) {

                $_SESSION['user'] = $model->select([], ['email' => $email])[0];
